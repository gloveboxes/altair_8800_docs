...
> In the following procedure, you're directly setting memory addresses. You might overwrite something important and crash the Altair emulator. If this happens, just [reboot the Altair](../20-fundamentals/24-Rebooting-the-Altair.md) and experiment again.
...
1. From the web terminal, select **Ctrl+M** to exit the CPU monitor. This will resume code execution from the current CPU instruction pointer. Depending on what you have been changing in memory, you may need to [reboot CP/M](../20-fundamentals/24-Rebooting-the-Altair.md).
